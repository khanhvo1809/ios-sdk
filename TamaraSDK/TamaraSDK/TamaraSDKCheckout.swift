...
extension TamaraSDKCheckout: WKNavigationDelegate {
    public func webView(
        ...
        if (url.absoluteString.contains(merchantURL.success)) {
            self.delegate?.onSuccessfull()
        }
        else if (url.absoluteString.contains(merchantURL.failure)) {
            self.delegate?.onFailured()
        }
        else if (url.absoluteString.contains(merchantURL.cancel)) {
            self.delegate?.onCancel()
        }
        else if (url.absoluteString.contains(merchantURL.notification)) {
